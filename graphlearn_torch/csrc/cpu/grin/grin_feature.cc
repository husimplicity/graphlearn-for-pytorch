...
torch::Tensor GrinVertexFeature::cpu_get(const torch::Tensor& ex_ids) {
  ...
  int64_t num_props = grin_get_vertex_property_list_size(graph_, props);
  torch::Tensor vfeats = torch::empty({bs, num_props}, torch::kFloat32);

  at::parallel_for(0, num_props, 1, [&](int32_t start, int32_t end) {
    for (size_t j = start; j < end; j++) {
      auto prop = grin_get_vertex_property_from_list(graph_, props, j);
      for (int32_t i = 0; i < bs; i++) {
        auto v = grin_get_vertex_by_external_id_of_int64(graph_, ex_ids_ptr[i]);
        vfeats[i][j] = grin_get_vertex_property_value_of_float(graph_, v, prop);
